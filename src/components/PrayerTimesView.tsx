...
export const PrayerTimesView: React.FC<PrayerTimesViewProps> = ({
  ...
  return (
    <div className="w-full space-y-2 sm:space-y-2.5 animate-fadeIn">
      {/* Hero Card: Next Prayer & Countdown */}
      <div className="w-full relative overflow-hidden rounded-2xl sm:rounded-3xl bg-gradient-to-br from-[#064e3b] via-[#065f46] to-[#042f2e] text-white shadow-lg border border-emerald-600/40 p-2.5 sm:p-3.5">
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between text-xs text-emerald-200/90 mb-1.5">
            <button
              onClick={onOpenSettings}
              className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald-950/50 border border-emerald-500/30 hover:bg-emerald-800/60 active:scale-95 transition font-medium shadow-xs text-xs"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold transition border active:scale-95 shadow-xs ${
                isPlayingAdhan
          ...
          <div className="text-center my-1.5 sm:my-2">
            <p className="text-[10.5px] sm:text-xs uppercase tracking-widest text-emerald-300 font-semibold mb-0.5 sm:mb-1 font-tajawal drop-shadow-xs">
              الصلاة القادمة
            </p>
            <h2 className="text-xl sm:text-2xl font-black text-white font-tajawal drop-shadow-md flex items-baseline justify-center gap-2">
              <span className="tracking-tight text-white">
                {effectiveNextPrayer?.nameAr || 'الفجر'}
              </span>
              <span className="text-sm sm:text-base font-medium text-emerald-100/90 font-mono" dir="ltr">
          ...
          <div className="my-1.5 sm:my-2 flex items-center justify-center gap-1.5" dir="ltr">
            <div className="flex flex-col items-center bg-emerald-950/60 backdrop-blur-xs border border-emerald-600/40 rounded-xl px-2.5 py-1 min-w-[54px] sm:min-w-[58px] shadow-inner">
              <span className="text-lg sm:text-xl font-bold font-mono tracking-tight text-amber-300 drop-shadow-xs">
                {countdown.hours}
              </span>
              <span className="text-[8.5px] text-emerald-200 font-tajawal font-medium">ساعة</span>
            </div>
            <span className="text-lg font-bold text-emerald-300/70 mb-1.5">:</span>
            <div className="flex flex-col items-center bg-emerald-950/60 backdrop-blur-xs border border-emerald-600/40 rounded-xl px-2.5 py-1 min-w-[54px] sm:min-w-[58px] shadow-inner">
              <span className="text-lg sm:text-xl font-bold font-mono tracking-tight text-amber-300 drop-shadow-xs">
                {countdown.minutes}
              </span>
              <span className="text-[8.5px] text-emerald-200 font-tajawal font-medium">دقيقة</span>
            </div>
            <span className="text-lg font-bold text-emerald-300/70 mb-1.5">:</span>
            <div className="flex flex-col items-center bg-emerald-950/60 backdrop-blur-xs border border-emerald-600/40 rounded-xl px-2.5 py-1 min-w-[54px] sm:min-w-[58px] shadow-inner">
              <span className="text-lg sm:text-xl font-bold font-mono tracking-tight text-amber-300 drop-shadow-xs">
              ...
              <span className="text-[8.5px] text-emerald-200 font-tajawal font-medium">ثانية</span>
            </div>
          </div>

          {/* Progress Bar */}
          <div className="mt-2">
            <div className="w-full bg-emerald-950/70 h-1.5 rounded-full overflow-hidden border border-emerald-700/50 p-[1px]">
              <div
            ...
            <div className="flex justify-between text-[9.5px] sm:text-[10px] text-emerald-200/90 mt-0.5 font-medium">
      ...
      <div className="w-full bg-white dark:bg-slate-800/95 rounded-2xl sm:rounded-3xl border border-emerald-100/80 dark:border-slate-700/60 shadow-xs p-2.5 sm:p-3">
        <div className="flex items-center justify-between pb-1.5 border-b border-slate-100 dark:border-slate-700/50 mb-1">
          <h3 className="font-bold text-xs sm:text-sm text-slate-900 dark:text-slate-100 font-tajawal">
            مواقيت الصلاة اليوم
          </h3>
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-700/40">
          {effectivePrayers.map((prayer) => {
            ...
            return (
              <div
                key={prayer.id}
                className={`py-1.5 px-2 rounded-lg sm:rounded-xl transition-all duration-200 flex items-center justify-between ${
                  prayer.isNext
                    ? 'bg-gradient-to-r from-emerald-800 to-emerald-700 text-white shadow-md my-0.5 border border-emerald-600/40'
                    : prayer.isPassed
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <div
                    className={`w-7.5 h-7.5 rounded-lg flex items-center justify-center transition-transform ${
                      prayer.isNext
                  ...
                  <div>
                    <div className="flex items-center gap-1.5">
                      <span className={`font-bold text-[13px] sm:text-sm font-tajawal ${prayer.isNext ? 'text-white' : 'text-slate-900 dark:text-slate-100'}`}>
                        {prayer.nameAr}
                      </span>
                      {prayer.isNext && (
                        <span className="px-1.5 py-0.5 rounded-full text-[8.5px] font-bold bg-amber-400 text-slate-950 shadow-xs">
                          الآن
                        </span>
                      )}
                    </div>
                    <span className={`text-[9.5px] font-medium ${prayer.isNext ? 'text-emerald-200' : 'text-slate-400'}`}>
                      {prayer.nameEn}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2" dir="ltr">
                  <button
                    ...
                    className={`p-0.5 rounded-md transition active:scale-95 ${
                      prayer.isNext
                        ? isAlertOn
                  ...
                  >
                    {isAlertOn ? <Bell className="w-3.5 h-3.5" /> : <BellOff className="w-3.5 h-3.5" />}
                  </button>
                  <span
                    className={`font-bold text-sm sm:text-base tracking-tight font-mono min-w-[2.9rem] text-right ${
